...
class Flow:
    ...
    def create_chrome(self):
        login_page = LibecityPage()  # ログインページ操作クラスのインスタンス化
        driver = login_page.create_chrome()  # ドライバ生成メソッドの呼び出し
        return driver  # ドライバを返す

    # ログインページにアクセスする
    def access_login_page(self, driver):
        login_page = LibecityPage()  # インスタンス化
        login_page.login_page(driver)  # ログインページに遷移

    # ログインIDの入力欄を探して入力する
    def type_email(self, driver, email):
        login_page = LibecityPage()  # インスタンス化
        login_page.input_email(driver).send_keys(email)  # メールアドレスを入力

    # パスワードの入力欄を探して入力する
    def type_password(self, driver, password):
        login_page = LibecityPage()  # インスタンス化
        login_page.input_password(driver).send_keys(password)  # パスワードを入力

    # ログインボタンを探してクリックする
    def click_login_button(self, driver):
        login_page = LibecityPage()  # インスタンス化
        login_page.login_button(driver).click()  # ログインボタンをクリック
    ...
    # 例外処理をまとめて実行する
    def exception_all(self):
        email, password = self.load_env()  # .envからメールアドレスとパスワードを取得
        if not email or not password:  # どちらかが未設定の場合
            print("環境変数が設定されていません")  # エラー表示
            return  # 処理を終了

        logger = Logger()  # ロガーのインスタンスを作成
        driver = self.create_chrome()  # Chromeドライバを起動

        try:
            self.access_login_page(driver)  # ログインページにアクセス
            self.type_email(driver, email)  # メールアドレスを入力
            self.type_password(driver, password)  # パスワードを入力
            self.click_login_button(driver)  # ログインボタンをクリック

            msg = ""  # メッセージ用変数を初期化
            if self.check_login(driver):  # ログイン成功か判定
                msg = f"✅ リベシティにログイン成功：{self.get_username(driver)}"  # 成功メッセージを作成
            else:
                msg = "⚠️ ログイン失敗：URLに dashboard または home が含まれていません"  # 失敗メッセージを作成
            print(msg)  # 結果を出力
            self.send_chatwork(msg, logger)  # チャットワークにメッセージを送信

        except Exception as e:
            error_msg = f"❌ エラー発生：{e}"  # エラーメッセージを作成
            print(error_msg)  # エラー内容を出力
            self.send_chatwork(error_msg, logger)  # チャットワークにエラーを送信

        finally:
            driver.quit()  # ブラウザを閉じる
            print("ブラウザを閉じました")  # 終了メッセージを出力
